...
int main() {
    ...
    long long t = 0;
    // calculate sum(every number to every number using lines). 
    ...
    // greedy approach to place line in required group 
    while (s > 0 || r > 0)
    {
        if (s > r)
        {
    ...
    // result -= sum(selected to selected) 
    for (int i = 0; i < sl.size(); i ++)
        t -= sl[i] * (k - 2*i - 1);
    // result -= sum(non-selected to non-selected) 
